...
def staff_take_pt(request):
    ...
    context = {
        'subjects': subjects,
        'sessions': sessions,
        'page_title': 'Take PT Meeting'
    }

    return render(request, 'staff_template/staff_take_pt.html', context)

def staff_view_pt(request):
    staff = get_object_or_404(Staff, admin=request.user)
    student_data = request.POST.get('student_ids')
    # date = request.POST.get('date')
    # students = json.loads(student_data)
    sessions = Session.objects.all()
    context = {
        'page_title': 'View PT Meeting'
    }

    return render(request, 'staff_template/staff_view_pt.html', context)
