...
def save_pt_meeting(request):
    if request.method == 'POST':
        try:
            ...
            data = json.loads(request.POST.get('ptMeetingData'))
            ...
            for entry in data:
                ...
                # Save data to the database (assuming you have a PTMeeting model)
                pt_meeting = PTMeeting(student=student, query=query, action=action, photo=photo)
                pt_meeting.save()
